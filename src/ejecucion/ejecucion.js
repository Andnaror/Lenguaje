...
const registro_1 = require("./instrucciones/registro");
...
const retorno_1 = require("./expresiones/flujo/retorno");
...
const Ytambien_1 = require("./expresiones/logicas/Ytambien");
const Obien_1 = require("./expresiones/logicas/Obien");
const No_1 = require("./expresiones/logicas/No");
...
const si_1 = require("./SI");
const instruccion_si_1 = require("./expresiones/condicionales/instruccion_si");
const mientrasq_1 = require("./instrucciones/ciclos/mientrasq");
const hacer_mientrasq_1 = require("./instrucciones/ciclos/hacer_mientrasq");
const para_1 = require("./instrucciones/ciclos/para");
...
class Ejecucion {
    ...
    recorrer(nodo) {
            ...
            switch (nodo.hijos.length) {
                ...
                case 2:
                    ...
                    if (nodo.hijos[0] == '!' && this.soyNodo('EXP', nodo.hijos[1])) {
                        ...
                        return new No_1.No(nodo.linea, exp);
                    }
                case 3:
                    //EXP mas EXP
                    ...
                    if (this.soyNodo('EXP', nodo.hijos[0]) && nodo.hijos[1] == '&&' && this.soyNodo('EXP', nodo.hijos[2])) {
                        ...
                        const expDer = this.recorrer(nodo.hijos[2]);
                        const linea = nodo.linea;
                        return new Ytambien_1.Ytambien(linea, expIzq, expDer);
                    }
                    //EXP or EXP
                    if (this.soyNodo('EXP', nodo.hijos[0]) && nodo.hijos[1] == '||' && this.soyNodo('EXP', nodo.hijos[2])) {
                        const expIzq = this.recorrer(nodo.hijos[0]);
                        const expDer = this.recorrer(nodo.hijos[2]);
                        const linea = nodo.linea;
                        return new Obien_1.Obien(linea, expIzq, expDer);
                    }
        ...
        //ESCRIBA_REGISTRO
        if (this.soyNodo('ESCRIBA_REGISTRO', nodo)) {
            //escriba punto registro par_izq LISTA_EXPRESIONES par_der punto_coma
            const lista = this.recorrer(nodo.hijos[4]);
            return new registro_1.Registro(nodo.linea, lista);
        }
        // ATRIBUTO
        ...
        if (this.soyNodo('RETURN', nodo)) {
            switch (nodo.hijos.length) {
                ...
                case 3:
                    const exp = this.recorrer(nodo.hijos[1]);
                    return new retorno_1.Retorno(nodo.linea, true, exp);
                //return punto_coma
                case 2:
                    return new retorno_1.Retorno(nodo.linea, false);
            }
        }
        //ARRAY_LENGTH
        ...
        if (this.soyNodo('INSTRUCCION_SI', nodo)) {
            switch (nodo.hijos.length) {
                ...
                case 1:
                    ...
                    return new instruccion_si_1.InstruccionSi(nodo.linea, [inst]);
                case 2:
                    //SI SINO
                    if (this.soyNodo('SI', nodo.hijos[0]) && this.soyNodo('SINO', nodo.hijos[1])) {
                        const inst_si = this.recorrer(nodo.hijos[0]);
                        const inst_sino = this.recorrer(nodo.hijos[1]);
                        return new instruccion_si_1.InstruccionSi(nodo.linea, [inst_si, inst_sino]);
                    }
                    //SI LISTA_SINO_SI
                    if (this.soyNodo('SI', nodo.hijos[0]) && this.soyNodo('LISTA_SINO_SI', nodo.hijos[1])) {
                        const inst_si = this.recorrer(nodo.hijos[0]);
                        const lista_sis = this.recorrer(nodo.hijos[1]);
                        return new instruccion_si_1.InstruccionSi(nodo.linea, [inst_si, ...lista_sis]);
                    }
                //SI LISTA_SINO_SI SINO
                case 3:
                    const inst_si = this.recorrer(nodo.hijos[0]);
                    const lista_sis = this.recorrer(nodo.hijos[1]);
                    const inst_sino = this.recorrer(nodo.hijos[2]);
                    return new instruccion_si_1.InstruccionSi(nodo.linea, [inst_si, ...lista_sis, inst_sino]);
            }
        }
        //SI
        if (this.soyNodo('SI', nodo)) {
            //SI par_izq EXP par_der llave_izq INSTRUCCIONES llave_der
            const condicion = this.recorrer(nodo.hijos[2]);
            const instrucciones = this.recorrer(nodo.hijos[5]);
            return new si_1.SI(condicion, instrucciones);
        }
        //SINO
        if (this.soyNodo('SINO', nodo)) {
            //SINO llave_izq INSTRUCCIONES llave_der
        ...
        if (this.soyNodo('SINO_SI', nodo)) {
            //SINO SI par_izq EXP par_der llave_izq INSTRUCCIONES llave_der
        ...
        //Mientrasq
        if (this.soyNodo('MIENTRASQ', nodo)) {
            //mientrasq par_izq EXP par_der llave_izq INSTRUCCIONES llave_der
            const condicion = this.recorrer(nodo.hijos[2]);
            const instrucciones = this.recorrer(nodo.hijos[5]);
            return new mientrasq_1.Mientrasq(nodo.linea, condicion, instrucciones);
        }
        //HACER_MIENTRASQ
        if (this.soyNodo('HACER_MIENTRASQ', nodo)) {
            //do llave_izq INSTRUCCIONES llave_der mientrasq par_izq EXP par_der punto_coma
            const instrucciones = this.recorrer(nodo.hijos[2]);
            const condicion = this.recorrer(nodo.hijos[6]);
            return new hacer_mientrasq_1.Hacer_mientrasq(nodo.linea, instrucciones, condicion);
        }
        //ASIGNACION_PARA
        ...
        if (this.soyNodo('PARA', nodo)) {
            const condicion = this.recorrer(nodo.hijos[3]);
            const asignacion_para = this.recorrer(nodo.hijos[5]);
            const instrucciones = this.recorrer(nodo.hijos[8]);
            //para par_izq DECLARACION_VARIABLE EXP punto_coma ASIGNACION_PARA par_der llave_izq INSTRUCCIONES llave_der
            if (this.soyNodo('DECLARACION_VARIABLE', nodo.hijos[2])) {
                ...
                return new para_1.Para(nodo.linea, declaracion, null, condicion, asignacion_para, instrucciones);
            }
            //para par_izq ASIGNACION EXP punto_coma ASIGNACION_PARA par_der llave_izq INSTRUCCIONES llave_der
            if (this.soyNodo('ASIGNACION', nodo.hijos[2])) {
                const asignacion = this.recorrer(nodo.hijos[2]);
                return new para_1.Para(nodo.linea, null, asignacion, condicion, asignacion_para, instrucciones);
            }
        }
        //FOR_OF
