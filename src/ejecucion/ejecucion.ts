...
import { Registro } from './instrucciones/registro';
...
import { Retorno } from './expresiones/flujo/retorno';
...
import { Ytambien } from './expresiones/logicas/Ytambien';
import { Obien } from './expresiones/logicas/Obien';
import { No } from './expresiones/logicas/No';
...
import { SI } from './SI';
import { InstruccionSi } from './expresiones/condicionales/instruccion_si';
import { Mientrasq } from './instrucciones/ciclos/mientrasq';
import { Hacer_mientrasq } from './instrucciones/ciclos/hacer_mientrasq';
import { Para } from './instrucciones/ciclos/para';
...
export class Ejecucion {
  ...
  recorrer(nodo: any): any {
      ...
      switch (nodo.hijos.length) {
        ...
        case 3:
          ...
          if (this.soyNodo('EXP', nodo.hijos[0]) && nodo.hijos[1] == '&&' && this.soyNodo('EXP', nodo.hijos[2])) {
            ...
            const expDer = this.recorrer(nodo.hijos[2]);
            const linea = nodo.linea;
            return new Ytambien(linea, expIzq, expDer);
          }
          //EXP or EXP
          if (this.soyNodo('EXP', nodo.hijos[0]) && nodo.hijos[1] == '||' && this.soyNodo('EXP', nodo.hijos[2])) {
            const expIzq = this.recorrer(nodo.hijos[0]);
            const expDer = this.recorrer(nodo.hijos[2]);
            const linea = nodo.linea;
            return new Obien(linea, expIzq, expDer);
          }
    ...
    if (this.soyNodo('ESCRIBA_REGISTRO', nodo)) {
      ...
      return new Registro(nodo.linea, lista);
    }
    ...
    if (this.soyNodo('RETURN', nodo)) {
      switch (nodo.hijos.length) {
        ...
        case 3:
          const exp = this.recorrer(nodo.hijos[1]);
          return new Retorno(nodo.linea, true, exp);
        //return punto_coma
        case 2:
          return new Retorno(nodo.linea, false);
      }
    }
    ...
    if (this.soyNodo('INSTRUCCION_SI', nodo)) {
      switch (nodo.hijos.length) {
        ...
        case 1:
          ...
          return new InstruccionSi(nodo.linea, [inst]);
        case 2:
          //SI SINO
        ...
        case 3:
          ...
          return new InstruccionSi(nodo.linea, [inst_si, ...lista_sis, inst_sino]);
      }
    }
    ...
    if (this.soyNodo('MIENTRASQ', nodo)) {
      ...
      const condicion = this.recorrer(nodo.hijos[2]);
      const instrucciones = this.recorrer(nodo.hijos[5]);
      return new Mientrasq(nodo.linea, condicion, instrucciones);
    }
    ...
    if (this.soyNodo('HACER_MIENTRASQ', nodo)) {
      ...
      return new Hacer_mientrasq(nodo.linea, instrucciones, condicion);
    }
    ...
    if (this.soyNodo('PARA', nodo)) {
      ...
      if (this.soyNodo('DECLARACION_VARIABLE', nodo.hijos[2])) {
        ...
        return new Para(nodo.linea, declaracion, null, condicion, asignacion_para, instrucciones);
      }
      ...
      if (this.soyNodo('ASIGNACION', nodo.hijos[2])) {
        ...
        return new Para(nodo.linea, null, asignacion, condicion, asignacion_para, instrucciones);
      }
    }
