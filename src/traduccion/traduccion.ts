...
export class Traduccion {
  ...
  recorrer(nodo: any, e: Entorno): any {
    ...
    //PARA
    else if (this.soyNodo('PARA', nodo)) {
      switch (nodo.hijos.length) {
        case 10:
          //para par_izq DECLARACION_VARIABLE EXP punto_coma ASIGNACION_FOR par_der llave_izq INSTRUCCIONES llave_der
          if (this.soyNodo('DECLARACION_VARIABLE', nodo.hijos[2])) {
            ...
            return `para(${dec_var} ${exp}; ${asig_for}){\n${instrucciones}}`;
          }
          //para par_izq ASIGNACION EXP punto_coma ASIGNACION_FOR par_der llave_izq INSTRUCCIONES llave_der
    ...
    //SINO
    else if (this.soyNodo('SINO', nodo)) {
    ...
    //SINO_IF
    else if (this.soyNodo('SINO_IF', nodo)) {
      //SINO if par_izq EXP par_der llave_izq INSTRUCCIONES llave_der
      const exp = this.recorrer(nodo.hijos[3], e);
      const entorno = new Entorno(e);
    ...
    //LISTA_SINO_IF
    else if (this.soyNodo('LISTA_SINO_IF', nodo)) {
      let codigoAux = '';
      nodo.hijos.forEach((nodoHijo: any) => {
        if (nodoHijo instanceof Object) {
          codigoAux += this.recorrer(nodoHijo, e);
